...
En este capítulo se fijan las definiciones que se utilizan en el resto del trabajo. Se introducen alfabetos, cadenas y lenguajes, así como operaciones y problemas de decisión asociados. Posteriormente se presentan las gramáticas formales y los autómatas finitos. Finalmente, se introducen nociones de complejidad computacional y el problema de satisfacibilidad booleana.
...
Al ser los lenguajes conjuntos de cadenas, es posible definir sobre ellos operaciones clásicas de conjuntos como unión, intersección y complemento \cite{automataTheory}. También se consideran problemas asociados a los lenguajes, como determinar si una cadena pertenece a un lenguaje o si un lenguaje es vacío.
...
La definición de lenguaje como conjunto de cadenas no especifica un mecanismo de generación. Para describir lenguajes mediante reglas finitas se introducen las gramáticas formales.
...
\begin{definition}{(Gramática)}
	...
	\begin{itemize}
		...
		\item $T$ es un conjunto de símbolos terminales. Debe cumplirse que $N \cap T = \emptyset$
...
\begin{definition}{(Derivación directa)}
	Sean $x,y \in (N \cup T)^*$. Se dice que $x$ deriva directamente en $y$ si existen $x_1, x_2 \in (N \cup T)^*$ y una regla $\alpha \gprod \beta$ en $G$ tales que
	$$
...
La derivación directa permite sustituir una subcadena de una cadena por otra según una regla de producción. A partir de esta operación se define la derivación en varios pasos.
...
\begin{definition}{(Lenguaje generado)}
	Sea $G$ una gramática, el lenguaje generado por $G$ es el siguiente conjunto:
	$$
		L(G) = \{x \in T^* \suchthat S \gderp x\}
	$$
\end{definition}

No todas las gramáticas o formalismos para generar lenguajes son iguales. La jerarquía de Chomsky \cite{hunter2020chomsky} clasifica las gramáticas en diferentes tipos según la forma de sus reglas de producción, lo que a su vez determina el tipo de lenguajes que pueden generar. Estas gramáticas se incluyen de forma estricta, es decir, cada tipo de gramática es un subconjunto estricto del tipo anterior:
...
\begin{itemize}
	...
	\item En las gramáticas de \textbf{tipo-1}, o gramáticas sensibles al contexto \text{CS}, las reglas de producción son de la forma $\alpha A \gamma \gprod \alpha \beta \gamma$, donde $A \in N$, $\alpha, \beta, \gamma \in (N \cup T)^*$, y $|\beta| \geq 1$. Estas generan los lenguajes dependientes del contexto.

	\item Las gramáticas de \textbf{tipo-2}, o gramáticas libres de contexto (\textit{CFG}), son ampliamente utilizadas en la construcción de compiladores. Sus reglas de producción son de la forma $A \gprod \beta$, donde $A \in N$ y $\beta \in (N \cup T)^*$. Estas generan los lenguajes libres de contexto, que son reconocidos por autómatas de pila \cite{automataTheory}.

	\item Finalmente, las gramáticas de \textbf{tipo-3}, o gramáticas regulares, son gramáticas con reglas de producción de la forma $A \gprod aB$ o $A \gprod a$, donde $A, B \in N$ y $a \in T$. Estas gramáticas generan los lenguajes regulares (\textit{REG}), que son reconocidos por autómatas finitos \cite{automataTheory}.
\end{itemize}

Un formalismo o gramática es más poderoso que otro si puede generar un conjunto de lenguajes que el otro no puede, o sea, la familia de lenguajes generados es mas grande.
...
% TODO: mejorar esta introducción temporal cuando tenga el capitulo
Los autómatas finitos deterministas permiten decidir la pertenencia a lenguajes regulares. Este mecanismo se utilizará para modelar lenguajes asociados a interpretaciones de fórmulas booleanas.

Los modelos de reconocimiento permiten decidir la pertenencia a lenguajes. En este contexto, resulta necesario considerar el costo computacional de estos problemas de decisión. A continuación se introducen nociones básicas de complejidad computacional.

\section{Complejidad computacional}
La teoría de la complejidad computacional clasifica problemas en función de los recursos necesarios para resolverlos.
...
Se dice que un algoritmo requiere una cantidad de tiempo polinomial si su tiempo de ejecución es $O(n^k)$ para alguna constante $k$ y entrada de tamaño $n$. La complejidad de un problema es la complejidad del algoritmo más eficiente que lo resuelve en el peor caso.
