...
% yo vi que Raudel hizo esto en su tesis, dijo algo así como "... esto es importante porque se lo usamos para no se que cosa GRAMÁTICAS DE CONCATENACIÓN DE RANGO (que es eso?), estas se definen en el capitulo TAL" si no fitduing, quitamos donde se define
...
Una derivación en una gramática consiste en seleccionar una regla de producción $\alpha \gder \beta$ y sustituir una ocurrencia de $\alpha$ en una cadena $w \in (N \cup T)^*$ por $\beta$ \cite{automataTheory}.
...
\begin{itemize}
	\item Las gramáticas tipo-0, también conocidas como gramáticas irrestrictas, son las más generales. Sus reglas no tienen restricciones (son de la forma $\alpha \gprod \beta$ donde $\alpha \in (N \cup T)^* - T^*$ y $\beta \in (N \cup T)^*$). El conjunto de lenguajes que generan se denomina lenguajes recursivamente enumerables (RE).

	\item En las gramáticas de tipo-1, o gramáticas dependientes del contexto (CS), las reglas de producción son de la forma $\alpha A \gamma \gprod \alpha \beta \gamma$, donde $A \in N$, $\alpha, \beta, \gamma \in (N \cup T)^*$, y $|\beta| \geq 1$. Estas generan los lenguajes dependientes del contexto (CS).
	...
	\item Finalmente, las gramáticas de tipo-3, o gramáticas regulares, son gramáticas con reglas de producción de la forma $A \gprod aB$ o $A \gprod a$, donde $A, B \in N$ y $a \in T$. Estas gramáticas generan los lenguajes regulares (REG).
\end{itemize}
...
Si se toma un caso particular de $\repeatlang$, al cual se le llama $\repeatlang^n = \{(wd)^n \suchthat w \in \binalph\}$ se cumple que $\repeatlang^1$ es un lenguaje regular, mientras que $\repeatlang^k$ para $k > 1$ es un lenguaje dependiente del contexto \cite{automataTheory}.
...
\begin{definition}{(Evaluación de una fórmula)}
	...
	% no estaría mal una referencia a un libro de lógica o algo de ese estilo
\end{definition}
