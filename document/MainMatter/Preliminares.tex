...
En esta sección se definen conceptos fundamentales y se presentan problemas relevantes de la teoría de lenguajes formales, la cual constituye la base matemática de áreas como la computación. Además se introducen los principales mecanismos que describen y reconocen lenguajes.
...
% quizás esté un poco corta esta introducción
...
\begin{definition}{(Derivación)}
	Sean $x,y \in (N \cup T)^*$. Se dice que $x$ deriva en $y$ en un número finito de pasos si existe una secuencia de cadenas
	$$
...
No todas las gramáticas son capaces de generar todos los lenguajes. La jerarquía de Chomsky \cite{hunter2020chomsky} clasifica las gramáticas en diferentes tipos según la forma de sus reglas de producción, lo que a su vez determina el conjunto de lenguajes que pueden generar. Estas gramáticas se incluyen de forma estricta, es decir, cada tipo de gramática es un subconjunto estricto del tipo anterior:
...
Un AFD procesa una cadena de entrada aplicando la función de transición $\delta$ para cada símbolo de la cadena comenzando en el estado inicial $q_0$. Si al finalizar el procesamiento de la cadena el autómata se encuentra en un estado que pertenece a $F$, entonces se dice que el autómata acepta la cadena, lo que implica que la cadena pertenece al lenguaje reconocido por el autómata.

Con esta función de transición extendida, se define el lenguaje reconocido por un AFD.
...
En dependencia de los recursos computacionales necesarios para resolver un problema, se agrupan los problemas en diferentes clases de complejidad. A continuación se presentan algunas de estas, las cuales clasifican problemas de decisión según el tiempo de ejecución de los algoritmos que los resuelven.
...
El problema de la satisfacibilidad booleana (SAT) es un problema de decisión fundamental en las ciencias de la computación. En esta sección se define este problema y se analiza su importancia para la teoría de la complejidad computacional
...
El problema de satisfacibilidad booleana se utiliza como punto de partida para los resultados que se desarrollan en este trabajo.
